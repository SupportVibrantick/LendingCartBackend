...
/** Products that collect Equipment Types on Loan Criteria (Asset Based Lending only). */
export const productUsesEquipmentTypes = (productCode?: string | null) =>
  isEquipmentFinanceProduct(productCode) ||
  isSba7aEquipmentPurchaseProduct(productCode);

/** Bridge / DSCR / Fix & Flip / Construction / CRE / Agency / Mezz-Pref / CMBS use property types only — not business types. */
export const productUsesAcceptedPropertyTypes = (
  productCode?: string | null,
) =>
  isBridgeLoanProduct(productCode) ||
  isDscrRentalProduct(productCode) ||
  isFixAndFlipProduct(productCode) ||
  isConstructionLoanProduct(productCode) ||
  isCrePermanentProduct(productCode) ||
  isAgencyMultifamilyProduct(productCode) ||
  isMezzOrPreferredEquityProduct(productCode) ||
  isCmbsProduct(productCode) ||
  isRentalPortfolioProduct(productCode) ||
  isCPaceProduct(productCode);

export const productUsesBusinessTypes = (productCode?: string | null) =>
  !productUsesAcceptedPropertyTypes(productCode);
...
export const validateLoanProductCriteriaStep = (
  ...
  for (const product of selectedProducts) {
    ...
    if (
      isBridgeLoanProduct(product.code) ||
      isCrePermanentProduct(product.code) ||
      isAgencyMultifamilyProduct(product.code) ||
      isMezzOrPreferredEquityProduct(product.code) ||
      isCmbsProduct(product.code) ||
      isRentalPortfolioProduct(product.code)
    ) {
...
export const buildLenderProductCriteriaPayload = (
  ...
  const cPaceProduct = isCPaceProduct(productCode);
  ...
  const payload = {
    ...
    nonOwnerOccupiedAllowed: occupancyBorrowerFlags
      ...
      : false,
    purchaseAllowed:
      occupancyBorrowerFlags || sba7aRealEstateProduct || cPaceProduct
    ...
    apartmentAllowed:
      crePermanentProduct || cmbsProduct
        ...
        : false,
    officeAllowed:
      crePermanentProduct || mezzOrPrefProduct || cmbsProduct || cPaceProduct
    ...
    medicalHealthcareAllowed:
      crePermanentProduct || cmbsProduct || cPaceProduct
    ...
    minDscrArm:
      ...
      criteria.minDscrArm !== ""
        ...
        : null,
    newConstructionAllowed: agencyMultifamilyProduct || cPaceProduct
    ...
    constructionAllowed: cPaceProduct
      ? Boolean(criteria.constructionAllowed)
      : false,
    energyEfficiencyImprovementsAllowed: cPaceProduct
      ? Boolean(criteria.energyEfficiencyImprovementsAllowed)
      : false,
    renewableEnergyImprovementsAllowed: cPaceProduct
      ? Boolean(criteria.renewableEnergyImprovementsAllowed)
      : false,
    waterEfficiencyImprovementsAllowed: cPaceProduct
      ? Boolean(criteria.waterEfficiencyImprovementsAllowed)
      : false,
    resiliencyImprovementsAllowed: cPaceProduct
      ? Boolean(criteria.resiliencyImprovementsAllowed)
      : false,
    seismicImprovementsAllowed: cPaceProduct
      ? Boolean(criteria.seismicImprovementsAllowed)
      : false,
    hvacBuildingSystemsAllowed: cPaceProduct
      ? Boolean(criteria.hvacBuildingSystemsAllowed)
      : false,
    solarRenewableEnergyAllowed: cPaceProduct
      ? Boolean(criteria.solarRenewableEnergyAllowed)
      : false,
    roofImprovementsAllowed: cPaceProduct
      ? Boolean(criteria.roofImprovementsAllowed)
      : false,
    lightingImprovementsAllowed: cPaceProduct
      ? Boolean(criteria.lightingImprovementsAllowed)
      : false,
    buildingEnvelopeAllowed: cPaceProduct
      ? Boolean(criteria.buildingEnvelopeAllowed)
      : false,
    propertyOwnerConsentRequired: cPaceProduct
      ? Boolean(criteria.propertyOwnerConsentRequired)
      : false,
    seniorLenderConsentRequired: cPaceProduct
      ? Boolean(criteria.seniorLenderConsentRequired)
      : false,
    mortgageLenderConsentRequired: cPaceProduct
      ? Boolean(criteria.mortgageLenderConsentRequired)
      : false,
    energyAuditRequired: cPaceProduct
      ? Boolean(criteria.energyAuditRequired)
      : false,
    cPaceEligibleJurisdictions: cPaceProduct
      ? (() => {
          const joined = Array.isArray(criteria.cPaceJurisdictions)
            ? criteria.cPaceJurisdictions.filter(Boolean).join(",")
            : criteria.cPaceEligibleJurisdictions?.trim() || "";
          return joined || null;
        })()
      : null,
  ...
  return {
    ...
    states: Array.isArray(product.statesSupported)
      ...
      : product.statesSupported
        ...
        : [],
    cPaceJurisdictions: Array.isArray(product.cPaceEligibleJurisdictions)
      ? product.cPaceEligibleJurisdictions
      : product.cPaceEligibleJurisdictions
        ? String(product.cPaceEligibleJurisdictions).split(",").filter(Boolean)
        : [],
    constructionAllowed: toFormBoolean(product.constructionAllowed),
    energyEfficiencyImprovementsAllowed: toFormBoolean(
      product.energyEfficiencyImprovementsAllowed,
    ),
    renewableEnergyImprovementsAllowed: toFormBoolean(
      product.renewableEnergyImprovementsAllowed,
    ),
    waterEfficiencyImprovementsAllowed: toFormBoolean(
      product.waterEfficiencyImprovementsAllowed,
    ),
    resiliencyImprovementsAllowed: toFormBoolean(
      product.resiliencyImprovementsAllowed,
    ),
    seismicImprovementsAllowed: toFormBoolean(product.seismicImprovementsAllowed),
    hvacBuildingSystemsAllowed: toFormBoolean(product.hvacBuildingSystemsAllowed),
    solarRenewableEnergyAllowed: toFormBoolean(
      product.solarRenewableEnergyAllowed,
    ),
    roofImprovementsAllowed: toFormBoolean(product.roofImprovementsAllowed),
    lightingImprovementsAllowed: toFormBoolean(
      product.lightingImprovementsAllowed,
    ),
    buildingEnvelopeAllowed: toFormBoolean(product.buildingEnvelopeAllowed),
    propertyOwnerConsentRequired: toFormBoolean(
      product.propertyOwnerConsentRequired,
    ),
    seniorLenderConsentRequired: toFormBoolean(
      product.seniorLenderConsentRequired,
    ),
    mortgageLenderConsentRequired: toFormBoolean(
      product.mortgageLenderConsentRequired,
    ),
    energyAuditRequired: toFormBoolean(product.energyAuditRequired),
