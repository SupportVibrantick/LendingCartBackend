...
const Chat = ({ applicationId, onBack }: LoanPreviewChatProps) => {
    ...
    async (search = debouncedSearch) => {
      ...
      try {
        ...
        setSelectedConversation((prev) => {
          if (prev) {
            return (
              nextConversations.find((c: Conversation) => c.id === prev.id) ||
              null
            );
          }
  ...
  const ensureClientOfficerConversation = async () => {
    ...
    const token = getToken();
    const res = await fetch(
      `${API_BASE}/messaging/conversations/client-officer`,
      {
        method: "POST",
        headers: {
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
      <div className="mb-2 flex shrink-0 items-center justify-between gap-3">
        <button
          type="button"
          onClick={onBack}
          className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-600 shadow-sm transition hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700"
        >
          <FiArrowLeft size={14} />
          Back to application
        </button>
      ...
      <div className="grid h-full min-h-0 flex-1 overflow-hidden rounded-[24px] border border-slate-200/80 bg-white shadow-[0_18px_50px_rgba(5,150,105,0.08)] lg:grid-cols-[330px_minmax(0,1fr)]">
        <aside
        ...
        >
          <div className="border-b border-slate-200/80 px-4 py-4">
            ...
            <div className="group relative">
              <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 transition-colors group-focus-within:text-emerald-600" />
              <input
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                placeholder="Search people or messages"
                className="h-11 w-full rounded-2xl border border-slate-200 bg-white pl-10 pr-10 text-sm text-slate-700 outline-none transition-all placeholder:text-slate-400 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/15"
              />
              {searchTerm ? (
                <button
                  type="button"
            ...
            {chatLoading ? (
              ...
              <div className="flex h-full flex-col items-center justify-center px-6 text-center">
                ...
                <p className="mt-2 max-w-[220px] text-xs leading-6 text-slate-400">
                  {debouncedSearch
                    ? "Try another keyword or contact name."
                    : "Your broker team chats will appear here."}
                </p>
              </div>
            ) : (
                  ...
                  return (
                    <motion.button
                      ...
                      className={`flex w-full items-center gap-3 rounded-2xl border px-3 py-3 text-left transition-all ${
                        isActive
                          ? "border-emerald-500/30 bg-emerald-500/8 shadow-sm ring-1 ring-emerald-500/15"
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 flex items-center justify-between gap-2">
                          ...
                          {chat.unread ? (
                            <span className="flex h-5 min-w-5 shrink-0 items-center justify-center rounded-full bg-emerald-600 px-1.5 text-[10px] font-semibold text-white">
                              1
                            </span>
                          ) : null}
                        </div>
        ...
        <section
          className={`flex h-full min-h-0 flex-col overflow-hidden bg-[linear-gradient(180deg,#f8fafc_0%,#ffffff_42%)] ${
          ...
          {!isChatSelected ? (
            <div className="flex h-full items-center justify-center px-6 text-center">
              <motion.div
                ...
                className="max-w-md"
              >
                <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl border border-slate-200 bg-white text-emerald-600 shadow-sm">
                ...
                <p className="mt-2 text-sm leading-7 text-slate-500">
                  Choose your broker or loan officer chat from the left panel.
                </p>
              </motion.div>
            </div>
          ) : (
            <>
              <div className="flex shrink-0 items-center justify-between border-b border-slate-200/80 bg-white/90 px-4 py-3 backdrop-blur sm:px-5">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="relative shrink-0">
                    <div
                      className={`flex h-11 w-11 items-center justify-center rounded-full text-xs font-semibold ring-2 ring-white ${getAvatarTone(contactDisplayName)}`}
                    >
                  ...
                  <div className="flex h-full items-center justify-center text-center">
                    <div className="rounded-2xl border border-dashed border-slate-200 bg-white/80 px-6 py-8">
                      ...
                      <p className="mt-2 text-xs text-slate-400">
                        Send a message to your broker team below.
                      </p>
                    </div>
                  </div>
                ) : (
                      ...
                      {messages.map((msg, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div
                              className={`flex py-1 ${!isBroker ? "justify-end" : "justify-start"} ${grouped ? "mt-0.5" : "mt-3"}`}
                            >
                              <div
                                className={`flex max-w-[min(88%,520px)] items-end gap-2 ${
                                  !isBroker ? "flex-row-reverse" : "flex-row"
                                }`}
                              >
                                {!grouped ? (
                                  <div
                                  ...
                                  >
                                    {getInitials(
                                      isBroker
                                        ? msg.senderName || contactDisplayName
                                        : "You",
                                    )}
                                  </div>
                                ) : (
                                  ...
                                  {!grouped ? (
                                    <div
                                      className={`mb-1 flex items-center gap-2 px-1 text-[10px] text-slate-400 ${
                                        !isBroker ? "justify-end" : ""
                                      }`}
                                    >
                                      <span
                                        className={`font-semibold ${!isBroker ? "text-emerald-600" : "text-slate-500"}`}
                                      >
                                        {isBroker
              ...
              <div className="relative shrink-0 border-t border-slate-200/80 bg-white/95 px-3 py-3 backdrop-blur sm:px-5">
                ...
                <div className="flex items-end gap-2 rounded-[22px] border border-slate-200 bg-white px-2 py-2 shadow-sm transition-all focus-within:border-emerald-500 focus-within:ring-2 focus-within:ring-emerald-500/15">
                  <button
                    type="button"
