...
function actionClass(action: string) {
  ...
  if (upper.includes("DELETE")) return "bg-red-50 text-red-700 ring-1 ring-inset ring-red-600/20";
  if (upper.includes("CREATE")) return "bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/20";
  if (upper.includes("UPDATE")) return "bg-blue-50 text-blue-700 ring-1 ring-inset ring-blue-600/20";
...
const AdminLogs: React.FC = () => {
  ...
  async function fetchLogs(page: number, limit: number) {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE}/broker/logs?page=${page}&limit=${limit}`, {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 
            ...
            <>
              <div className="overflow-x-auto">
               ...
               border-b border-slate-100 dark:border-slate-800">
                      ...
                      <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Event</th>
                      <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Entity</th>
                      <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">User / IP</th>
                      <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 text-right">Data</th>
                    </tr>
                  </thead>
               ...
               transition-colors group">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-2 text-slate-600">
                            ...
                            <span className="text-sm font-medium dark:text-slate-200">{formatDateTime(log.createdAt)}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex flex-col gap-1.5">
                            <span className={`inline-flex items-center w-fit px-2.5 py-0.5 rounded-full text-xs font-bold ${actionClass(log.action)}`}>
                              {log.action}
                            </span>
                            <span className="text-xs text-slate-400 font-medium uppercase tracking-tight">{log.category}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                ...
                transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                            ...
                            <div className="text-xs text-slate-400 ml-5">{log.ipAddress}</div>
                          </div>
                        </td>
      ...
      {selectedLog && (
        <div className="fixed inset-0 z-[9797979790] flex items-center justify-center p-4 
