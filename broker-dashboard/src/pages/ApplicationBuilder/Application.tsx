  ...
  return (
    <div className="flex justify-between items-center border p-2 rounded bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700">
      ...
      <div>
        ...
        <div className="text-xs text-slate-400">{uiType}</div>
      </div>
...
export default function ApplicationBuilder() {
  ...
  async function saveFieldToServer(field: FormField) {
    ...
    /* ================= UPDATE MODE ================= */
    if (!editingId) payload.sectionId = selectedSectionId;
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/broker/applications/fields/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  return (
    <div className="min-h-screen p-6 bg-slate-50 text-slate-900 dark:bg-slate-900 dark:text-slate-100">
      <h1 className="text-2xl font-bold mb-6 text-[#2C92D5]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        {/* APPLICATION */}
        <select
          className="text-sm border rounded-lg px-3 py-2 bg-white text-slate-900 border-slate-300 dark:bg-slate-800 dark:text-slate-100 dark:border-slate-600"
          value={selectedAppId}
        ...
        {selectedProductId && (
          <select
            className="text-sm w-full border rounded px-3 py-2 bg-white text-slate-900 border-slate-300
      ...
      {!selectedAppId || !selectedProductId ? (
        <div className="flex flex-col items-center justify-center py-20 text-center border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 max-w-md">
            ...
            from above to start building your dynamic form.
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-6">
          ...
          <div className="border rounded-xl p-5 space-y-4 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700">
            ...
            <select
              className="text-sm w-full border rounded px-3 py-2 bg-white text-slate-900 border-slate-300 dark:bg-slate-800 dark:text-slate-100 dark:border-slate-600"
              value={form.type}
              onChange={(e) =>
            ...
            <div className="flex items-center gap-3 mt-2">
              <input
                ...
                className="text-sm h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
              />
              <label
                htmlFor="required"
            ...
            {form.type !== "select" &&
              ...
              form.type !== "checkbox" && (
                <input
                  className="text-sm w-full border rounded-lg px-3 py-2 bg-white dark:bg-slate-800 dark:border-slate-600"
            ...
            <button
              ...
              className="bg-[#2C92D5] text-white px-4 py-2 rounded text-xs"
            >
          ...
          <form className="border rounded-xl p-5 space-y-6 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700">
            {sections.map((section) => (
              <div key={section.id}>
                <h3 className="text-sm font-semibold mb-4 text-slate-700 dark:text-white bg-slate-100 dark:bg-slate-600 rounded-sm px-2 py-1">
                  {section.name}
                </h3>
               ...
               resize-none"
                        ...
                        {["text", "email", "number", "date"].includes(
                          ...
                          <input
                            ...
                            readOnly
                            placeholder={f.placeholder || ""}
                            className="w-full rounded-lg border px-3 py-2 text-xs bg-white dark:bg-slate-800 dark:border-slate-600"
                          />
                        )}
                      </div>
                    );
