...
export default function FeeAgreement({ applicationId, getAuthHeaders }: Props) {
  ...
  const fetchAgreement = async () => {
    try {
      ...
      const res = await fetch(
        `${API_BASE}/loanofficer/loan-pipeline/${applicationId}/fee-agreement`,
        {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const res = await fetch(
        `${API_BASE}/loanofficer/loan-pipeline/${data.id}/fee-agreement`,
        {
          method: "PATCH",
          headers: {
            ...getAuthHeaders(),
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <FeeAgreementDownloadButton
            data={data}
            pdfRef={pdfRef}
            downloadUrl={`${API_BASE}/loanofficer/loan-pipeline/${applicationId}/fee-agreement/download-pdf`}
