...
export default function ViewContactModal({ contact, onClose }: Props) {
  const Field = ({ label, value }: { label: string; value?: string }) => (
    <div className="flex flex-col gap-1">
      ...
      <span className="text-xs font-medium text-gray-800 bg-blue-50 p-2 rounded-md">
        {value || "-"}
      </span>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[999999]">
      ...
      <div className="bg-white w-[650px] max-h-[80vh] rounded-2xl shadow-xl p-6 relative flex flex-col">
