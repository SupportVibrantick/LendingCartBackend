  ...
  fastify.put(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        const formatted = normalizeLenderProductForAdminApi(updated, {
          documents: mapLenderDocumentRequirements(
            updated.lenderDocumentRequirements,
          ),
        });
        ...
        return reply.send({
          ...
          data: {
            ...formatted,
            code: updated.loanProduct?.code || updated.loanProductCode,
            name: updated.loanProduct?.name || null,
            loanProductId: updated.loanProductId,
            minLoanAmount: formatted.minLoanAmount
              ? Number(formatted.minLoanAmount)
              : null,
            maxLoanAmount: formatted.maxLoanAmount
              ? Number(formatted.maxLoanAmount)
              : null,
            minTermMonths: updated.minTermMonths,
            maxTermMonths: updated.maxTermMonths,
            minCreditScore: updated.minCreditScore,
            businessTypes: formatted.businessTypes,
            propertyTypes: formatted.propertyTypes,
            statesSupported: formatted.statesSupported,
            equipmentTypes: formatted.equipmentTypes,
          },
        });
      } catch (error) {
        req.log.error(error);
