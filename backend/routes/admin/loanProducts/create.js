    ...
    async (req, reply) => {
      ...
      try {
        ...
        if (!validation.success) {
          return reply.status(400).send({ success:false, message:"Invalid input", details: validation.error });
        }

        const { code, name, description } = validation.data;

        const exists = await prisma.loanProduct.findFirst({ where:{ code }});
        if (exists) {
          return reply.status(409).send({ success:false, message:"Loan product already exists" });
        }
        ...
        return reply.status(201).send({ success:true, message:"Loan product created", data:product });
      } catch (error) {
