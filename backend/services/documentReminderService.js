...
const { notifyClient, CLIENT_NOTIFICATION_EVENTS } = require("./clientNotifications");
const { notifyLender, LENDER_NOTIFICATION_EVENTS } = require("./lenderNotifications");
...
async function sendDocumentReminderEmail(prisma, schedule, loan, pendingItems) {
  ...
  if (schedule.recipientType === "CLIENT") {
    ...
    const { buildLenderLoanPreviewUrl } = require("../utils/emailBranding");
