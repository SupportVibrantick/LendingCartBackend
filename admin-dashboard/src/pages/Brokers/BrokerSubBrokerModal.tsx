...
import {
  ...
  fetchBrokerOrgLoanOfficersForSubBroker,
  fetchBrokerSubBrokerDetail,
  updateBrokerSubBroker,
  type BrokerSubBrokerLoanOfficer,
...
export default function BrokerSubBrokerModal({
  ...
  const [loanOfficers, setLoanOfficers] = useState<BrokerSubBrokerLoanOfficer[]>(
    [],
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const [officersRes, loanTypesRes] = await Promise.all([
          fetchBrokerOrgLoanOfficersForSubBroker(brokerId),
          fetch(`${ADMIN_API_BASE}/common/loan-products/loan-product-code`, {
            headers: {
        ...
        const loanTypesJson = await loanTypesRes.json();
        if (!loanTypesRes.ok || !loanTypesJson.success) {
          throw new Error(loanTypesJson.message || "Failed to load loan types");
        }
        setLoanTypeOptions(
          (loanTypesJson.data || []).map(
    ...
    if (!isEdit || !subBrokerId) {
      ...
      (async () => {
        try {
          ...
          const mapped = mapDetailToForm(json.data as CoBrokerDetail);
          if (mapped.logoPreview && !mapped.logoPreview.startsWith("http")) {
            mapped.logoPreview = `${ADMIN_API_BASE}${mapped.logoPreview}`;
          }
          setForm(mapped);
        } catch (err) {
          if (!cancelled) {
            toast.error(
              err instanceof Error ? err.message : "Failed to load co-broker",
            );
            onClose();
          }
        } finally {
  ...
  }, [brokerId, isEdit, isOpen, onClose, subBrokerId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      const formData = buildBrokerSubBrokerFormData(form);

      if (isEdit) {
        if (!subBrokerId) throw new Error("Missing co-broker id");
        await updateBrokerSubBroker(brokerId, subBrokerId, formData);
      } else {
        await createBrokerSubBroker(brokerId, formData);
      }
  ...
  return (
    <div
      className="fixed inset-0 z-[9999999] flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    >
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    {loanOfficerOptions.length
                      ...
                      : "Create loan officers under this broker organization to assign them here."}
                  </p>
                </div>
              </section>
            </div>
