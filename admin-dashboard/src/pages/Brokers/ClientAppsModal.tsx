...
import { ChevronLeft, ChevronRight, Contact, ExternalLink, FileText, Loader2, Search, User, X } from "lucide-react";
import toast from "react-hot-toast";
import {
  fetchBrokerClientApplications,
...
export default function ClientAppsModal({
  brokerId,
  client,
  onClose,
  formatDate,
  ...
  const [total, setTotal] = useState(client.applicationsCount ?? 0);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const json = await fetchBrokerClientApplications(brokerId, client.id, {
          page,
          limit: PAGE_SIZE,
  ...
  }, [brokerId, client.id, onClose, page, debouncedSearch]);
  ...
  return (
    <div className="fixed inset-0 z-[999999] flex items-center justify-center bg-black/40 p-4 backdrop-blur-[1px]">
      <div className="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
        <div className="h-1 bg-gradient-to-r from-cyan-600 via-teal-500 to-[#13538A] opacity-80" />
        ...
        <div className="flex items-start justify-between gap-3 border-b border-slate-100 px-5 py-4 dark:border-slate-800">
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">Client Applications</h3>
              <p className="mt-0.5 truncate text-[11px] text-slate-500">{clientName}</p>
              {client.primaryContact?.email ? (
                <p className="truncate text-[10px] text-slate-400">{client.primaryContact.email}</p>
              ) : null}
            </div>
          </div>
          <button
        ...
        <div className="border-b border-slate-100 px-5 py-3 dark:border-slate-800">
          <div className="flex flex-wrap items-center gap-2">
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 dark:border-slate-700 dark:bg-slate-800/60">
              ...
              <p className="text-sm font-bold text-cyan-700">{total}</p>
            </div>
            {totalAmount > 0 ? (
              <div className="rounded-lg border border-cyan-200 bg-cyan-50 px-3 py-1.5 dark:border-cyan-900/40 dark:bg-cyan-950/30">
                <p className="text-[9px] font-semibold uppercase tracking-wide text-cyan-600 dark:text-cyan-400">
        ...
        <div className="border-b border-slate-100 px-5 py-3 dark:border-slate-800">
          <div className="relative max-w-md">
            ...
            <input
              ...
              placeholder="Search application #, product, status..."
              className="w-full rounded-lg border border-slate-200 bg-white py-2 pl-9 pr-3 text-xs text-slate-700 outline-none transition focus:border-cyan-400 focus:ring-2 focus:ring-cyan-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:focus:ring-cyan-900/40"
            />
          </div>
        </div>
            ...
            <div className="flex flex-1 flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50/50 px-6 py-14 text-center dark:border-slate-700 dark:bg-slate-800/30">
              ...
              <p className="mt-1 max-w-xs text-[11px] text-slate-500">
                {debouncedSearch
                  ...
                  : "This client does not have any applications for this broker."}
              </p>
            </div>
          ) : (
                    ...
                    {applications.map((row) => (
                      ...
                      >
                        <td className="px-3 py-3">
                          ...
                          >
                            <p className="inline-flex items-center gap-1 font-semibold text-cyan-700 group-hover:underline">
