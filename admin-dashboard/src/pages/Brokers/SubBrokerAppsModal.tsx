...
import { ChevronLeft, ChevronRight, ExternalLink, FileText, Loader2, Search, User, UsersRound, X } from "lucide-react";
import toast from "react-hot-toast";
import {
  fetchBrokerSubBrokerApplications,
...
export default function SubBrokerAppsModal({
  brokerId,
  subBroker,
  onClose,
  formatDate,
  ...
  const [total, setTotal] = useState(subBroker.assignedApplications ?? 0);
  const [totalPages, setTotalPages] = useState(1);
  const [totalAmount, setTotalAmount] = useState(0);
  const [viewApplicationId, setViewApplicationId] = useState<string | null>(null);

  const subBrokerName =
    [subBroker.firstName, subBroker.lastName].filter(Boolean).join(" ") ||
    subBroker.email ||
    "Sub-Broker";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const json = await fetchBrokerSubBrokerApplications(brokerId, subBroker.id, {
          page,
          limit: PAGE_SIZE,
  ...
  }, [brokerId, subBroker.id, onClose, page, debouncedSearch]);
  ...
  return (
    <div className="fixed inset-0 z-[999999] flex items-center justify-center bg-black/40 p-4 backdrop-blur-[1px]">
      <div className="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
        <div className="h-1 bg-gradient-to-r from-emerald-600 via-[#18B6B4] to-[#13538A] opacity-80" />
        ...
        <div className="flex items-start justify-between gap-3 border-b border-slate-100 px-5 py-4 dark:border-slate-800">
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">Assigned Applications</h3>
              <p className="mt-0.5 truncate text-[11px] text-slate-500">{subBrokerName}</p>
              {subBroker.email ? (
                <p className="truncate text-[10px] text-slate-400">{subBroker.email}</p>
              ) : null}
            </div>
          </div>
          <button
        ...
        <div className="border-b border-slate-100 px-5 py-3 dark:border-slate-800">
          <div className="flex flex-wrap items-center gap-2">
            <div className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1.5 dark:border-slate-700 dark:bg-slate-800/60">
              <div className="flex h-7 w-7 items-center justify-center rounded-full bg-emerald-500/10 text-[10px] font-bold text-emerald-700">
                {getInitials(subBroker.firstName, subBroker.lastName, subBroker.email)}
              </div>
              <div>
                <p className="text-[9px] font-semibold uppercase tracking-wide text-slate-400">Sub-broker</p>
                <p className="text-[11px] font-medium text-slate-800 dark:text-slate-100">{subBrokerName}</p>
              </div>
            </div>
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 dark:border-slate-700 dark:bg-slate-800/60">
              ...
              <p className="text-sm font-bold text-emerald-700">{total}</p>
            </div>
            {totalAmount > 0 ? (
        ...
        <div className="border-b border-slate-100 px-5 py-3 dark:border-slate-800">
          <div className="relative max-w-md">
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-200 bg-white py-2 pl-9 pr-3 text-xs text-slate-700 outline-none transition focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:focus:ring-emerald-900/40"
            />
          </div>
        </div>
            ...
            <div className="flex flex-1 flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50/50 px-6 py-14 text-center dark:border-slate-700 dark:bg-slate-800/30">
              ...
              <p className="mt-1 max-w-xs text-[11px] text-slate-500">
                {debouncedSearch
                  ...
                  : "This sub-broker does not have any assigned applications for this broker."}
              </p>
            </div>
          ) : (
                    ...
                    {applications.map((row) => (
                      ...
                      >
                        <td className="px-3 py-3">
                          ...
                          >
                            <p className="inline-flex items-center gap-1 font-semibold text-emerald-700 group-hover:underline">
