...
const ImpersonateBrokers = () => {
  ...
  const fetchBrokers = useCallback(async () => {
    ...
    try {
      ...
      setBrokers(Array.isArray(res.data?.data) ? res.data.data : []);
      setTotal(nextTotal);
      ...
      toast.error("Failed to load broker portals.");
      setBrokers([]);
      setTotal(0);
      setTotalPages(1);
    } finally {
  ...
  const handleViewPortal = async (orgId: string) => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        const portalUrl = `${BROKER_URI}/impersonate?token=${token}&user=${encodedUser}`;
        ...
        <div className="relative flex flex-col gap-5 p-6 md:flex-row md:items-center md:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 max-w-xl text-sm text-slate-500 dark:text-slate-400">
                Search active broker organizations and open their portal in a
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            <div className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2 dark:border-slate-700 dark:bg-slate-800/80">
              <LayoutGrid className="h-4 w-4 text-[#13538A]" />
              <div className="leading-none">
                <p className="text-[10px] font-medium uppercase tracking-wide text-slate-400">
                  {debouncedSearch ? "Matches" : "Brokers"}
                </p>
        ...
        <div className="flex flex-col gap-3 border-b border-slate-100 p-4 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between sm:p-5">
          <div className="relative min-w-0 flex-1 sm:max-w-md">
            ...
            <input
              ...
              className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-4 text-sm text-slate-800 outline-none transition focus:border-[#13538A] focus:bg-white focus:ring-2 focus:ring-[#13538A]/15 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:focus:border-sky-500"
              value={searchQuery}
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              className="h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none transition focus:border-[#13538A] focus:ring-2 focus:ring-[#13538A]/15 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"
          ...
          <table className="w-full min-w-[640px] border-collapse text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                          {debouncedSearch
                            ...
                            : "Broker organizations will appear here once available."}
                        </p>
                      </div>
                      {searchQuery && (
                        <button
                          ...
                          className="mt-1 text-sm font-medium text-[#13538A] hover:underline dark:text-sky-400"
                        >
                          Clear search
                        </button>
                      )}
                ...
                brokers.map((broker) => {
                  const isEntering = enteringId === broker.organizationId;
                  return (
                    <tr
                    ...
                    >
                      <td className="px-5 py-4 sm:px-6">
                        <div className="flex items-center gap-3.5">
                          <div
                            className={`relative flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-gradient-to-br text-sm font-bold text-white shadow-sm ring-2 ring-white dark:ring-slate-900 ${avatarTone(broker.name)}`}
                          >
                            {broker.profileImage ? (
                              <img
                          ...
                          <div className="min-w-0">
                            <p className="truncate font-semibold text-slate-900 dark:text-white">
                              {broker.name || "Untitled organization"}
                            </p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4 text-right sm:px-6">
                        <button
                          ...
                          onClick={() =>
                            handleViewPortal(broker.organizationId)
                          }
                          className="inline-flex items-center gap-2 rounded-xl bg-[#13538A] px-3.5 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-[#0f4573] disabled:cursor-not-allowed disabled:opacity-60 dark:bg-sky-600 dark:hover:bg-sky-500"
                        >
                          {isEntering ? (
                            <>
