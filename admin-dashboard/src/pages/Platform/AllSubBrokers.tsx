...
export default function AllSubBrokers() {
  const [rows, setRows] = useState<SubBrokerRow[]>([]);
  ...
  const fetchRows = useCallback(async () => {
    ...
    try {
      ...
      const json = await adminFetch<PaginatedResponse<SubBrokerRow[]>>(
        `/admin/sub-brokers?${params.toString()}`,
      );
      setRows(json.data || []);
  ...
  const stats = useMemo(() => {
    ...
    const apps = rows.reduce((sum, row) => sum + (row.assignedApplications || 0), 0);
    const loggedIn = rows.filter((row) => row.lastLoginAt).length;
    return { active, apps, loggedIn };
  }, [rows]);

  const toggleStatus = async (row: SubBrokerRow) => {
    const active = row.status === "ACTIVE";
    setRowLoadingId(row.id);
    try {
      await adminFetch(`/admin/sub-brokers/${row.id}/status`, {
        method: "PATCH",
        body: JSON.stringify({ status: active ? "DISABLED" : "ACTIVE" }),
      });
      toast.success(active ? "Sub-broker suspended" : "Sub-broker activated");
      fetchRows();
    } catch (err: any) {
  ...
  const removeRow = async (row: SubBrokerRow) => {
    ...
    setRowLoadingId(row.id);
    try {
      await adminFetch(`/admin/sub-brokers/${row.id}`, { method: "DELETE" });
      toast.success("Sub-broker removed");
      fetchRows();
    } catch (err: any) {
  ...
  return (
    <>
      <PageMeta
        title="Sub-Brokers"
        description="All sub-broker users across broker organizations"
      />

      <div className="space-y-6">
        {/* Header */}
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-br from-amber-600 via-orange-600 to-[#5D28A8] p-6 text-white shadow-lg dark:border-slate-800">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <div className="mb-2 inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1 text-xs font-medium backdrop-blur-sm">
                ...
                Platform Admin · Sub-Brokers
              </div>
              <h1 className="text-2xl font-semibold tracking-tight">Sub-Brokers</h1>
              <p className="mt-1 max-w-2xl text-sm text-white/80">
                Manage sub-broker accounts under principal brokers — track assigned applications,
                login activity, and account status platform-wide.
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                ...
                { label: "Assigned Apps", value: stats.apps, icon: FileText },
                { label: "Logged In", value: stats.loggedIn, icon: UserCheck },
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-4 text-sm outline-none focus:border-amber-500/40 focus:ring-2 focus:ring-amber-500/10 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100"
              />
            </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              <div className="rounded-full bg-amber-100 p-4 text-amber-600 dark:bg-amber-500/10">
              ...
              <p className="mt-1 max-w-md text-sm text-slate-500">
                {search || statusFilter
                  ...
                  : "Sub-brokers will appear here once brokers add them to their organization."}
              </p>
            </div>
          ) : (
            <>
                ...
                <table className="min-w-[920px] w-full text-sm">
                  <thead className="sticky top-0 bg-slate-50/95 text-left text-xs uppercase tracking-wider text-slate-500 backdrop-blur dark:bg-slate-950/95">
                    <tr className="border-b border-slate-200 dark:border-slate-800">
                      <th className="px-5 py-3.5 font-semibold">Sub-Broker</th>
                      <th className="px-5 py-3.5 font-semibold">Principal Broker</th>
                      <th className="px-5 py-3.5 font-semibold">Assigned Apps</th>
