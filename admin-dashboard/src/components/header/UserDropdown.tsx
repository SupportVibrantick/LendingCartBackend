...
function resolveDisplayName(user: AdminSessionUser) {
...
export default function UserDropdown() {
  ...
  useEffect(() => {
    function persistSession(user: AdminSessionUser & Record<string, unknown>) {
      try {
        const raw = sessionStorage.getItem("admin_user");
        const existing = raw ? JSON.parse(raw) : {};
        const merged = { ...existing, ...user };
        sessionStorage.setItem("admin_user", JSON.stringify(merged));
        const name = resolveDisplayName(merged);
    ...
    function applyUser(user: AdminSessionUser) {
      const name = resolveDisplayName(user);
      setDisplayName(name || "Admin");
      if (user?.email) setDisplayEmail(user.email);

      const remote = buildAvatarUrl(user?.profileImage);
      if (remote) {
        setAvatarSrc(
          `${remote}${remote.includes("?") ? "&" : "?"}v=${encodeURIComponent(user.profileImage || "")}`,
        );
      } else {
        setAvatarSrc(
          `https://ui-avatars.com/api/?name=${encodeURIComponent(name || "Admin")}&background=13538A&color=ffffff`,
        );
      }
    }
    ...
    try {
      ...
      if (raw) {
        ...
        setAvatarSrc(
          `https://ui-avatars.com/api/?name=${encodeURIComponent(tokenName || "Admin")}&background=13538A&color=ffffff`,
        );
      }
    } catch {
      /* ignore */
    ...
    const refreshFromApi = async () => {
      ...
      try {
        ...
        persistSession(json.user);
        applyUser(json.user);
        window.dispatchEvent(
          new CustomEvent("admin-profile-updated", { detail: json.user }),
        );
      } catch {
    ...
    const onProfileUpdated = (e: Event) => {
      const detail = (e as CustomEvent<AdminSessionUser>).detail;
  ...
  return (
    <div className="relative">
      <button
        ...
        aria-haspopup="true"
      >
        <span className="mr-3 h-11 w-11 overflow-hidden rounded-full bg-slate-100 dark:bg-gray-800">
          <img
            src={avatarSrc}
            alt={displayName}
      ...
      >
        <div className="flex items-center gap-3 px-1 pb-1">
          <span className="h-10 w-10 overflow-hidden rounded-full bg-slate-100 dark:bg-gray-800">
            <img
              src={avatarSrc}
              alt={displayName}
              className="h-full w-full object-cover"
